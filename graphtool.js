...
function getCurveColor(id, o, hex) {
    ...
    if (hex) {
        let rgb = d3.rgb(hex);
        let hcl = d3.hcl(rgb);
        hcl.h = hcl.h + -270 * (o <= 0 ? 0 : 1); // Haruto's Bias, if you want to make the color more dicernable, change the 30 to 60, for the original, change the entire thing to 30 * o
        return hcl;
    } else {
...
let baseTitle = typeof page_title !== "undefined" ? page_title : "Fossy Graph Tool";
