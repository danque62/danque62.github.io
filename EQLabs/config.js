...
const init_phones = ["Flat Target"],                // Optional. Which graphs to display on initial load. Note: Share URLs will override this set
      ...
      watermark_text = "EQ Labs",                           // Optional. Watermark appears behind graphs
      watermark_image_url = "../fossy-logo.svg",   // Optional. If image file is in same directory as config, can be just the filename
      page_title = "Fossy Audio Labs",                     // Optional. Appended to the page title if share URLs are enabled
      page_description = "Test Targets and EQ for Target Creation",
...
const  preference_bounds_name = "Preference Bounds RAW",            // Preference bounds name
       ...
       default_DF_name = "Flat",                                        // Default RAW DF name
...
function watermark(svg) {
    ...
    if ( watermark_text ) {
        wm.append("text")
            .attrs({x:0, y:60, "font-size":24, "text-anchor":"middle", "class":"graph-name"})
            .text(watermark_text);
    }
...
const 
    ...
    simpleAbout = `
        ...
        <br>
        This "database" has no measurements of any headphones. This is intended for the user to upload their own measurements
        using any official rig that's supported.
        </p>
    `,
